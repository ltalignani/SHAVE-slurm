...
def get_threads(rule, default):
    ...
    elif rule in cluster_config and "cpus" in cluster_config[rule]:
        return int(cluster_config[rule]["cpus"])
    elif "__default__" in cluster_config and "cpus" in cluster_config["__default__"]:
        return int(cluster_config["__default__"]["cpus"])
...
#        GATK4
    ...
    resources:
        mem_mb=2000,
    #shell:
    #    "gatk CombineGVCFs -R {input.ref} -I {input.gvcfs} -O {output.gvcfs} &> {log} 2>&1 || true"
    wrapper:
...
rule indelrealigner:
 ...
 #       GATK
    ...
    log:
        "results/11_Reports/indelrealigner/{sample}_{aligner}.out.txt",
    params:
    ...
    resources:
        mem_mb=2000,
    #shell:
    #    "gatk -T IndelRealigner -I {input.bam} -R {input.ref} --targetIntervals {input.target_intervals} {params.extra} --out {output.bam} "
    wrapper:
...
#        GATK
    ...
    #shell:
    #    "gatk -T RealignerTargetCreator --num_threads {threads} -R {input.ref} -I {input.bam} {params.extra} -o {output.intervals} " 
    wrapper:
...
rule mark_duplicates_spark:
    message:
        "MarkDuplicatesSpark for {wildcards.sample} sample reads ({wildcards.aligner})"
    input:
    ...
    log:
        "results/11_Reports/samtools/{sample}_{aligner}_sorted-mark-dup.out.txt",
    params:
...
rule samtools_view:
    ...
    log:
        "results/11_Reports/samtools_view/{sample}_{aligner}.out.txt",
    params:
        extra="",  # optional params string
        region="",  # optional region string
    threads: get_threads('samtools_view', 8),
    shell:
        "samtools view -b -o {output.bam} {input} > {log} 2&1 "
    #wrapper:
