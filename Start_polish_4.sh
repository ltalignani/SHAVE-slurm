...
#SBATCH --job-name=SHAVE_POLISH4
#SBATCH --output=shave_polish4.%A.out
#SBATCH --error=shave_polish4.%A.err
...
###Charge module
module purge
module load samtools/1.15.1 snakemake/7.7.0 slurm-drmaa/1.0.8 picard/2.23.5 multiqc/1.13 graphviz/2.40.1
...
echo -e "${green}-----------------${nc}"
echo ""
echo -e "${blue}Name${nc} __________________ Start_polish_4.sh"
...
echo -e "${blue}Run${nc} ___________________ bash Start_polish_4.sh"
...
snakemake \
    --profile slurm/ \
    --directory ${workdir}/ \
    --snakefile workflow/rules/shave_polish_4.smk \
    --rerun-incomplete \
    --unlock \
    --use-conda \
