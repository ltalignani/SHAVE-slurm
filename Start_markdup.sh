...
#SBATCH --job-name=SHAVE_MARKDUP
#SBATCH --output=shave_markdup.%A.out
#SBATCH --error=shave_markdup.%A.err
...
# USAGE: sbatch Start_markdup.sh

###Charge module
module purge
module load samtools/1.15.1 snakemake/7.7.0 slurm-drmaa/1.0.8 gatk4/4.2.6.1 java-jdk/8.0.112 graphviz/2.40.1
...
snakemake \
    --directory ${workdir}/ \
    --snakefile workflow/rules/shave_markdup.smk \
    --config os=${os} \
    --rerun-incomplete \
    --unlock \
    --use-conda \
...
snakemake --directory ./ --snakefile workflow/rules/shave_markdup.smk --jobs=250 --cores 24 --config os="Linux" --rerun-incomplete --keep-going --conda-frontend conda --printshellcmds --drmaa " --account=aedes_amplicon --partition={cluster.partition} --mem={cluster.mem} -c {cluster.cpus} -o {cluster.output} -e {cluster.error}" --cluster-config cluster.yaml --configfile config.yaml --use-envmodules --use-conda
